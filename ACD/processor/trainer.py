...
from .analyze_model import analyze_layer_importance, swin_transformer_analyze_layer_importance
...
class BaseTrainer(object):
    def __init__(self, cfg, distiller, student_train_loader, distillation_loader, query_loader, gallery_loader):
        ...
        for name, param in self.distiller.teacher.named_parameters():
            param.requires_grad = False


        # 获取不重要的中间层名称
        if cfg.DISTILLER.TEACHER_NAME == "Swin_Transformer_V2_Small":
            inter_unimportant_block_names, nonInter_unimportant_block_names = swin_transformer_analyze_layer_importance(self.distiller,  self.distillation_loader, self.device, self.cfg)
        
        else:
            inter_unimportant_block_names, nonInter_unimportant_block_names = analyze_layer_importance(self.distiller,  self.distillation_loader, self.device, self.cfg)
   

        # 打印不重要层
        print(f"Unimportant Intersection layers (to train): {inter_unimportant_block_names}")
        print(f"Unimportant Non_Intersection layers (to train): {nonInter_unimportant_block_names}")

        
        # if cfg.DATASETS.NAMES == "CUB200" and cfg.INPUT.TEACHER_SIZE_TRAIN == [512, 512]:
        #     target_prefixes = ('layer3', 'layer4', 'fc')
        #     for name, param in self.distiller.teacher.named_parameters():
        #         param.requires_grad = name.startswith(target_prefixes)
        # else:
        # 直接全开，不用循环两遍
        for param in self.distiller.teacher.parameters():
            param.requires_grad = True

        # for name, param in self.distiller.teacher.named_parameters():
        #     if 'fc' in name:  # 假设最后一层名为 'fc'
        #         param.requires_grad = False  # 冻结全连接层
        #     else:
        #         param.requires_grad = True   # 其他层保持可训练

        # for name, param in self.distiller.teacher.named_parameters():
        #     layer_prefix = ".".join(name.split(".")[:2])
        #     if layer_prefix in inter_unimportant_block_names or layer_prefix in  nonInter_unimportant_block_names:
        #         param.requires_grad = True
        #     else:
        #         param.requires_grad = False
          
        self.optimizer = make_optimizer(cfg, distiller, inter_unimportant_block_names, nonInter_unimportant_block_names)
