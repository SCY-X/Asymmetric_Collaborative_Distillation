...
_C.DISTILLER.STUDENT_NAME = "ResNet101"
...
_C.DISTILLER.TEACHER_RATIO = 0.3
_C.DISTILLER.TEACHER_ALPHA = 1.0
_C.DISTILLER.TEACHER_BETA = 1.0
_C.DISTILLER.TEACHER_TEMPERATURE = 2
_C.DISTILLER.CROSS_RESOLUTION = True 
# # -----------------------------------------------------------------------------
...
# FITNET CFG
_C.FITNET = CN()
_C.FITNET.KD_WEIGHT = 1.0

# SSRL CFG
_C.SSRL = CN()
_C.SSRL.KD_WEIGHT = 1.0
_C.SSRL.SR_WEIGHT = 1.0
_C.SSRL.FEAT_WEIGHT = 1.0
...
# Default input settings (used when no specific student/teacher setting is referenced)

# Training image size
_C.INPUT.SIZE_TRAIN = [64, 64]
# Testing image size
_C.INPUT.SIZE_TEST = [64, 64]
# Padding applied to default input (ratio reference: 32→1, 64→2, 256→8, 384→12)
_C.INPUT.PADDING = 2

# ---------------------- Student Input Settings ----------------------

# Student training image size
_C.INPUT.STUDENT_SIZE_TRAIN = [64, 64]
# Student testing image size
_C.INPUT.STUDENT_SIZE_TEST = [64, 64]
# Padding applied to student input (ratio reference: same as above)
_C.INPUT.STUDENT_PADDING = 2

# ---------------------- Teacher Input Settings ----------------------

# Teacher training image size
_C.INPUT.TEACHER_SIZE_TRAIN = [256, 256]
# Teacher testing image size
_C.INPUT.TEACHER_SIZE_TEST = [256, 256]
# Padding applied to teacher input (ratio reference: 32→1, 64→2, 256→8, 384→12)
_C.INPUT.TEACHER_PADDING = 8
...
#the time learning rate of unimportant layer
_C.SOLVER.UNIMPORTANT_LR_TIMES = 1.0
