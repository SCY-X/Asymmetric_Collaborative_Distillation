...
from .datasets import CUB200, InShop, MSMT17, SOP, VeRi776, INat2021, DeepFashion, VERI_Wild, ImageNet, mini_imagenet
...
class DataLoaderFactory:
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.factory = {'CUB200': CUB200, 'InShop': InShop, 'SOP': SOP, 'VeRi776': VeRi776, 'MSMT17': MSMT17, 
                        'iNaturalist2021': INat2021, "DeepFashion": DeepFashion, 'VERI_Wild': VERI_Wild, 
                        "ImageNet": ImageNet, "mini_imagenet": mini_imagenet}
        
        # Define transforms
        self.train_transforms = self._build_train_transforms(self.cfg.INPUT.SIZE_TRAIN, self.cfg.INPUT.PADDING)
        self.kd_s_train_transforms = self._build_train_transforms(self.cfg.INPUT.STUDENT_SIZE_TRAIN, self.cfg.INPUT.STUDENT_PADDING)
        self.kd_t_train_transforms = self._build_train_transforms(self.cfg.INPUT.TEACHER_SIZE_TRAIN, self.cfg.INPUT.TEACHER_PADDING)
    ...
    def create_dataloaders(self):
        ...
        if self.cfg.DISTILLER.TYPE == "NONE":
            ...
            query_set = ImageDataset(self.dataset.query, self.gallery_transforms)
            gallery_set = ImageDataset(self.dataset.gallery, self.gallery_transforms)
            
        else:
            # Distillation loader
            distillation_train_set = Distillation_ImageDataset(
                self.dataset.train, self.kd_s_train_transforms, self.kd_t_train_transforms)
